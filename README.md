...
- 매일매일 걷거나 달리거나 조용히 앉아서 생각을 정리하고 기록할 수 있습니다.
  - CoreMotion 프레임워크의 API를 활용해 걷기/달리기 모드일 때의 PedometerData(만보기 데이터)를 수집하였습니다.
  - Time 구조체를 구현하여 유저가 생각을 기록하는 시간을 측정하였습니다.
- 기록한 생각 데이터는 로컬 데이터베이스에 저장되어 앱을 사용하는 동안 모아볼 수 있습니다.
  - RealmSwift 라이브러리를 활용하여 로컬 데이터베이스 객체를 구성하여 앱이 유지되는 동안 영구적으로 저장되도록 구현했습니다.
  - 저장된 데이터의 조회/수정/삭제하는 로직을 처리하는 서비스 객체를 활용하여 캘린더, 타임라인 뷰로 기록 데이터를 보여주었습니다.
  - 저장된 데이터를 바 그래프, 수치화 된 뷰로 확인할 수 있습니다.
